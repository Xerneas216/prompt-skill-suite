...
新鲜上下文的动态触发门禁独立于确定性验证。v0.3.0 的记录结果已经通过：隐式 Prompt 请求 `5/5`、直接任务负例 `5/5`、显式 `$procraft` `1/1`。逐用例路由证据位于 `evals/results/procraft-v0.3.0-trigger-run.json`。这证明的是本组触发边界，不代表模型整体质量已经验证。
